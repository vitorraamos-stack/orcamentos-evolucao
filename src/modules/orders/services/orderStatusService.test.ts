...
describe("transitionOrderStatus", () => {
  ...
  it("forwards payload context only as audit metadata; the server remains authoritative", async () => {
    const move = vi.fn().mockResolvedValue({ data: { id: "order" }, error: null });
    await transitionOrderStatus({ orderId: "order", board: "production", from: "Produção", to: "Em Acabamento", context: { role: "producao" } }, move);
    expect(move).toHaveBeenCalledWith(expect.objectContaining({
      p_event_payload: { board: "production", from: "Produção", to: "Em Acabamento" },
    }));
  });
});
