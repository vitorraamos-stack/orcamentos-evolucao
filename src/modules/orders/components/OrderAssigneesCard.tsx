...
export function OrderAssigneesCard({ assignees, users, canEdit, onChange }: { assignees: OrderAssignee[]; users: UserOption[]; canEdit: boolean; onChange: (scope: AssigneeScope, userId: string) => void }) {
  return <Card><CardHeader><CardTitle>Responsáveis</CardTitle></CardHeader><CardContent className="space-y-3">{scopes.map(scope => {
    ...
    return <div key={scope.value} className="grid gap-1 sm:grid-cols-[140px_1fr] sm:items-center"><span className="text-sm text-muted-foreground">{scope.label}</span>{canEdit ? <Select value={assignee?.user_id} onValueChange={value => onChange(scope.value, value)}><SelectTrigger><SelectValue placeholder="Nenhum responsável definido" /></SelectTrigger><SelectContent>{users.map(user => <SelectItem key={user.id} value={user.id}>{user.name}</SelectItem>)}</SelectContent></Select> : <span className="text-sm font-medium">{assignee?.user?.name ?? "Nenhum responsável definido para esta etapa."}</span>}</div>;
  })}</CardContent></Card>;
}
