...
export default function OsDetailPage() {
  ...
  const queryParamKiosk = new URLSearchParams(window.location.search).get(
    "kiosk"
  );
  const { user, hasModuleAccess } = useAuth();
  const isKioskMode =
    queryParamKiosk === "1" && hasModuleAccess("hub_os_kiosk");
  ...
  const handleOpenLayout = () => {
    ...
    setLayoutAsset(layoutCandidates[0] ?? null);
  ...
  const loadData = async () => {
    ...
    try {
      ...
      const layoutsPromise = fetchOsLayouts(osId, 5);
      ...
      const [eventData, paymentData, layouts] = await Promise.all([
        fetchOsEvents(osId),
  ...
  const handleSaveDetails = async () => {
    ...
    try {
      ...
      const payload = {
        ...
        shipping_carrier:
          deliveryType === "ENTREGA" ? shippingCarrier || null : null,
        tracking_code: deliveryType === "ENTREGA" ? trackingCode || null : null,
        address:
          deliveryType === "ENTREGA" || deliveryType === "INSTALACAO"
            ? address || null
            : null,
        notes: deliveryType === "ENTREGA" ? notes || null : null,
        installation_date:
          deliveryType === "INSTALACAO" ? installationDate || null : null,
        installation_time_window:
          deliveryType === "INSTALACAO" ? installationTimeWindow || null : null,
        on_site_contact:
          deliveryType === "INSTALACAO" ? onSiteContact || null : null,
  ...
  const handlePaymentSubmit = async () => {
    ...
    if (!parsed.success) {
      toast.error(
        parsed.error.issues[0]?.message ?? "Confira os dados do pagamento."
      );
      return;
    }
    ...
    try {
      ...
      if (paymentFile) {
        ...
        const presignData = await invokeEdgeFunction<{
          uploadUrl: string;
          bucket?: string;
        ...
        if (!uploadResponse.ok) {
          ...
          throw new Error(
            `Falha ao enviar arquivo (HTTP ${uploadResponse.status}): ${responseBody || "sem detalhes"}`
          );
        }
  ...
  if (loading) {
    return (
      <div className="text-muted-foreground">Carregando detalhes da OS...</div>
    );
  }
                ...
                <CardContent className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                  <div>
                    <p className="text-sm font-medium">
                      {layoutAsset.original_name || "Layout enviado"}
                    </p>
                    <p className="text-xs text-muted-foreground">
                      Enviado em {formatDateTime(layoutAsset.uploaded_at)}
                    </p>
                  </div>
                  <Button size="lg" onClick={handleOpenLayout}>
                    Abrir layout
                  </Button>
                </CardContent>
              </Card>
                    ...
                    <div className="rounded-md border p-4">
                      ...
                      <div className="grid gap-3 md:grid-cols-3">
                        ...
                        <div>
                          ...
                          <p className="font-medium">
                            {order.installation_time_window || "—"}
                          </p>
                        </div>
                        <div>
                          <p className="text-xs text-muted-foreground">
        ...
        {layoutAsset ? (
          <Card className="lg:col-span-2">
            ...
            <CardContent className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
              ...
              <Button onClick={handleOpenLayout}>Ver layout</Button>
            </CardContent>
          </Card>
        ) : null}
          ...
          <CardContent className="space-y-4">
              ...
              <div className="grid gap-4 md:grid-cols-2">
                ...
                <div className="space-y-1">
                  ...
                  <Input
                    ...
                    onChange={event =>
                      setInstallationTimeWindow(event.target.value)
                    }
                ...
                <div className="space-y-1 md:col-span-2">
                  ...
                  <Input
                    value={onSiteContact}
                    onChange={event => setOnSiteContact(event.target.value)}
                  />
                </div>
              </div>
            )}
            <div className="grid gap-4 md:grid-cols-2">
              <div className="flex items-center gap-2">
                <Checkbox
                  checked={isReproducao}
                  onCheckedChange={checked => setIsReproducao(Boolean(checked))}
                />
                <Label>Reprodução</Label>
              </div>
              <div className="flex items-center gap-2">
                <Checkbox
                  checked={hasLetraCaixa}
                  onCheckedChange={checked =>
                    setHasLetraCaixa(Boolean(checked))
                  }
                />
              ...
              {isReproducao && (
                <div className="space-y-1 md:col-span-2">
                  ...
                  <Input
                    value={reproMotivo}
                    onChange={event => setReproMotivo(event.target.value)}
                  />
                </div>
              )}
            </div>
            <div className="flex flex-wrap items-center gap-3">
              ...
              <Badge variant="outline">
                Status financeiro: {order.payment_status}
              </Badge>
              <span className="text-sm text-muted-foreground">
                Total orçamento: {formatCurrency(order.quote_total)}
              </span>
            </div>
          </CardContent>
        </Card>
          ...
          <CardContent className="space-y-4">
            ...
            <div className="space-y-2">
              ...
              <Select
                ...
                onValueChange={handleStatusProducaoChange}
              >
                <SelectTrigger>
                  <SelectValue placeholder="Selecione o status" />
                </SelectTrigger>
          ...
          <CardContent className="space-y-4">
            ...
            <div className="flex items-center gap-2">
              <Checkbox
                ...
                onCheckedChange={checked =>
                  setCadastroCompleto(Boolean(checked))
                }
              />
