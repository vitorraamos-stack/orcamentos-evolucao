...
const EVENT_OPTIONS = [
  ...
  { value: "returned_to_art", label: "OS retornada para Arte" },
  { value: "production_tag_changed", label: "Tag de Produção alterada" },
  { value: "archive", label: "Arquivamento" },
  { value: "delete", label: "Exclusão" },
  {
    value: "FINANCE_INSTALLMENT_STATUS_CHANGED",
    label: "Financeiro (status parcela)",
  },
];

const EVENT_LABELS = EVENT_OPTIONS.reduce<Record<string, string>>(
  (accumulator, option) => {
...
const formatDetails = (event: OsOrderEvent) => {
  ...
  if (event.type === "returned_to_art") {
    return `OS retornada para Arte: ${String(payload?.from_prod_status ?? payload?.from_art_status ?? "—")} → ${String(payload?.to_art_status ?? "Caixa de Entrada")}`;
  }
  if (event.type === "production_tag_changed") {
    return `Tag de Produção alterada: ${String(payload?.from ?? "—")} → ${String(payload?.to ?? "—")}`;
  }
...
export default function OsAuditPage() {
  ...
  const userOptions = useMemo(
    () => users.filter(user => user.full_name || user.email),
    [users]
  );
    ...
    <div className="space-y-6">
      <div className="space-y-1">
        ...
        <p className="text-sm text-muted-foreground">
          Visualize todas as movimentações e ações realizadas pelos usuários no
          Hub OS.
        </p>
      </div>
      ...
      <div className="grid gap-3 md:grid-cols-2 lg:grid-cols-4">
        ...
        >
          <SelectTrigger>
            <SelectValue
              placeholder={loadingUsers ? "Carregando usuários..." : "Usuário"}
            />
          </SelectTrigger>
          <SelectContent>
