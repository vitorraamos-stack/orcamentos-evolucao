...
select has_function('public', 'hub_os_send_to_production_secure', array['uuid','timestamp with time zone','date','jsonb']);
...
select function_privs_are('public', 'hub_os_send_to_production_secure', array['uuid','timestamp with time zone','date','jsonb'], 'authenticated', array['EXECUTE']);
...
select ok(not has_function_privilege('anon', 'public.hub_os_send_to_production_secure(uuid,timestamptz,date,jsonb)', 'EXECUTE'), 'anon cannot hand off to production');
...
select function_lang_is('public', 'hub_os_send_to_production_secure', array['uuid','timestamp with time zone','date','jsonb'], 'plpgsql');
